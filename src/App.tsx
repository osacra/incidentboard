...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav" aria-label="Navegação principal"><p className="nav-label">Workspace</p><button className={`nav-item ${activeView === 'overview' ? 'active' : ''}`} onClick={() => setActiveView('overview')}><span>▦</span> Visão geral</button><button className={`nav-item ${activeView === 'incidents' ? 'active' : ''}`} onClick={() => setActiveView('incidents')}><span>◈</span> Incidentes <b>{metrics.active}</b></button></nav>
